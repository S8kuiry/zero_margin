        ...
        <nav className="hidden lg:flex items-center gap-8">
          {Menu.map((item) => (
            <a
              key={item.id}
              href={item.link}
              className="text-lg font-medium text-white hover:text-orange-500 transition-all duration-300"
            >
              {item.name}
            </a>
          ))}
        ...
        <div className="flex items-center gap-4">
          <div className="relative hidden md:flex items-center">
            <input
              type="text"
              placeholder="Search..."
              className="w-48 md:w-56 lg:w-64 transition-all duration-300 border border-white/30 bg-white/10 text-white placeholder-white/60 rounded-full px-4 py-2 focus:ring-2 focus:ring-orange-500 outline-none"
            />
            <IoMdSearch className="absolute right-3 text-white/70" />
          </div>
          ...
          <button
            ...
            className="bg-blue-500 hover:bg-blue-700 transition-all duration-300 text-white font-medium px-5 py-2 rounded-full shadow-lg"
          >
            Switch Website
          </button>
        </div>
